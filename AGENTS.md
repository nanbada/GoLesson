...
- `docs/GoLesson AI용 추천 구조 및 개발자료.md` is history only. Do not treat it as SSOT.
...
3. Do not guess APIs, DB shape, or GoAlimi behavior. Read actual local/GitHub code first.
...
- Parser changes must be checked against fixed sentences in `docs/10_ACCEPTANCE_TEST.md` and `aidd_docs/fixtures/mvp-seed-data.md`.
...
aidd_docs/ plans, fixtures, session memory
```
