...
- [ ] A1. 프론트 Cloudflare Pages 배포 — 운영 env(publishable key)로 빌드 (docs/09 §4.2). 주의 1: 이전 로컬 빌드가 example.supabase.co를 번들에 구운 이력 있음 → real env 재빌드 + SW 캐시 클리어 확인. 주의 2: 빌드 Node 버전 고정 — `web/.node-version` 또는 Pages env `NODE_VERSION`으로 `web/package.json` engines(node>=20.9.0) 충족 보장.
- [ ] A2. (선택) Supabase secrets `OPENAI_MODEL_PARSE`, `OPENAI_MODEL_REPORT` 등록 — 운영 모델 고정용. 코드 기본값 존재(parse-batch/index.ts:423 gpt-4.1-nano, generate-report/index.ts:274 gpt-4.1-mini). `OPENAI_API_KEY`는 이미 등록됨(2026-07-06, Codex `supabase secrets list` 확인) → B2는 A2 없이 실행 가능.
- [ ] A3. 학원 PC 설치: GoAlimi `f9df186` 이후로 최신화 + Bridge 설치(bridge_config.json, run_bridge.bat, Task Scheduler ONLOGON) (docs/09 §4.3) — B3/B4 전제. 주의: GoAlimi requirements.txt에 greenlet 없음(T12 하니스에서 async 기동에 필요했음) → 최신화 후 기동 확인, 실패 시 PC venv에 greenlet>=3,<4 설치 또는 GoAlimi repo requirements 반영.
...
- [ ] B2. T5-2 — AI 의견이 입력 코멘트 밖의 사실·수치를 만들지 않는지 확인. API key 등록돼 있어 지금 실행 가능(A2 불필요) — 실제 AI 호출/품질/로그로 검증.
...
- ~~D3. GoAlimi repo에 greenlet requirements 반영~~ — A3 전제로 승격(2026-07-06, Codex 검토: T12 하니스에서 필수 확인, requirements.txt 부재).
