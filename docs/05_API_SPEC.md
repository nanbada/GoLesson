...
| 주체 | 경로 | 인증 |
|---|---|---|
| 프론트(PWA) CRUD | supabase-js 직접 (RLS), 다중 테이블 저장은 PostgREST RPC | Supabase Auth 세션 |
...
일반 CRUD(학생 조회, 교재·스케줄 관리 등)는 별도 API 없이 supabase-js + RLS로 처리한다.
다만 수업 저장(lesson/progress/homework/comment/parse_log)과 수강료 저장(payments/payment_items)은 부분 저장을 막기 위해 PostgREST RPC 한 번으로 처리한다.
...
- 저장은 하지 않는다. 확정 저장은 프론트가 `save_lesson_log` RPC로 수행(미리보기 원칙, REQ-504).
- 각 줄을 parse_logs에 기록(status=parsed/failed, created_by=요청자). 확정 저장 시 `save_lesson_log` RPC가 해당 parse_log status를 confirmed로 갱신한다 — 이를 위해 parse_logs는 `update(status)` 컬럼 GRANT + 본인 행(created_by=auth.uid()) RLS만 예외 허용한다 (04 §5 5-1). 응답에 각 줄의 parse_log_id를 포함해 프론트가 이를 참조할 수 있도록 함.
- 각 결과에는 warning 필드(예: 역행 경고 "역행: 42→38", 역순 구간 정리 안내)와 AI 결과의 confidence("high"|"low") 필드가 있을 수 있음.
...
### 2.4 PostgREST RPC (프론트 트랜잭션)

#### 2.4.1 POST /rest/v1/rpc/save_lesson_log

수업 화면 저장과 빠른입력 확정 저장을 한 트랜잭션으로 처리한다.

- payload: `lesson_id?`, `student_id`, `subject`, `schedule_slot_id?`, `lesson_date`, `progress?`, `carryover[]`, `new_homework?`, `comment?`, `parse_log_id?`
- 처리: lesson 생성/완료 또는 기존 lesson 완료 → progress insert(from/to 자동 정렬) → 이월 과제 체크 → 신규 과제 insert → comment insert → parse_log confirmed.
- 권한: authenticated 전용, SECURITY INVOKER. RLS/GRANT를 우회하지 않는다.

#### 2.4.2 POST /rest/v1/rpc/save_payment_with_items

수강료 저장을 한 트랜잭션으로 처리한다.

- payload: `payment_id?`, `student_id`, `paid_on`, `method`, `memo?`, `items[]`
- 처리: payment insert/update → 기존 payment_items 삭제 → 새 payment_items insert.
- 중간 실패 시 payment와 payment_items 변경은 모두 롤백된다.
- 권한: authenticated 전용, SECURITY INVOKER. RLS/GRANT를 우회하지 않는다.
