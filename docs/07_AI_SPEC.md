...
입력 텍스트 → 줄 분리(빈 줄 무시)
  └ 줄마다:
    ...
    ③ 과제 regex 추출
    ④ 남은 텍스트 = 코멘트
    ⑤ 필수(학생) + 1개 이상(진도|과제|코멘트) 성립 → regex 성공
    ⑥ 실패 → AI 호출 (1줄 단위)
...
- 후처리: student_name·textbook_title을 다시 사전 매칭해 id 확정 (AI가 낸 이름을 그대로 믿지 않는다). confidence=low면 카드에 "확인 필요" 배지.
...
- 정형 입력 파싱 정확도 95%+ (10_ACCEPTANCE §T5에 테스트 문장 20개 고정).
