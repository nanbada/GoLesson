...
> 소규모 초등 영어/수학 시간대별 학습·1:1 코칭 운영도구. ERP가 아니다.
...
| 수업 편성·운영 | 반은 학년·과목·레벨이 아니라 **요일+수업 시작 시각**으로 묶음. 시간대당 약 5명이 40분 수업 후 40분 과제·보강을 하며, 강사는 전 구간에서 학생별 5~10분 1:1 코칭 | 별도 class 마스터 없이 같은 시각의 `schedule_slots`를 오늘 화면에서 한 블록으로 묶고, `lesson` 상태로 학생별 코칭 진행을 표시 |
