...
| # | 절차 | 통과 기준 |
|---|---|---|
| 1 | 같은 시각 학생 약 5명의 스케줄로 폰에서 PWA 열기 | 3초 내 오늘 화면, 시작 시각별 한 블록으로 표시. 학년·과목이 달라도 같은 시각이면 함께 표시 |
| 2 | 블록의 학생 행 탭 → [1:1 코칭 시작] | 직전 수업 요약 표시, 해당 학생만 코칭 중으로 바뀌고 블록 완료 수는 유지 |
| 3 | 진도 to에 직전보다 큰 값 입력 → 저장 | 2초 내 저장, from은 자동이었음 |
| 4 | 과제 1건 추가(학원내) → 완료 체크+코멘트 | 저장됨 |
| 5 | [코칭 기록 완료] | 해당 학생이 코칭 완료로 바뀌고 블록 완료 수 증가. 오늘 진도·과제 결과가 학생 행에 표시 |
| 6 | 나머지 학생을 순서대로 기록 | 한 학생 완료가 다른 학생 상태를 바꾸지 않으며 코칭 완료 n/전체가 정확 |
| 7 | 전체 소요 측정 | 학생당 기록 조작 합계 30초 이내 (REQ-902) |
...
| 5 | 고정 테스트 문장 (§3) | 초기 10개 전수 통과. 인터뷰로 20개 확정 후 5요소 정확 매핑 19/20 이상 (95%, REQ-508) |
